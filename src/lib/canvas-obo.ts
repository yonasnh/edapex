...
interface LTIClaims {
  sub: string // Canvas user ID
  iss: string // Canvas issuer
...
export class CanvasOBOManager {
  private tokenCache = new Map<string, { token: OBOToken; expiresAt: number }>()

  /**
   * Exchange LTI claims for Canvas API access token
   */
  async exchangeForAccessToken(claims: LTIClaims): Promise<OBOToken> {
   ...
   */
  async getCanvasApiClient(claims: LTIClaims): Promise<CanvasAPIClient> {
...
export class CanvasAPIClient {
  ...
  async getUser(userId: string = 'self') {
