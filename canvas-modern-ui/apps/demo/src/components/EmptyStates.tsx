    ...
    return (
      ...
      >
        <div className="empty-state__content">
          ...
          {(action || secondaryAction) && (
            <div className="empty-state__actions">
              ...
              {secondaryAction && (
                <Button
                  kind={secondaryAction.variant || 'secondary'}
                  size={size === 'sm' ? 'sm' : 'md'}
        ...
        <div className="sr-only">
          ...
          {action && ` Primary action available: ${action.label}.`}
          {secondaryAction && ` Secondary action available: ${secondaryAction.label}.`}
        </div>
      </div>
    )
  }
 ...
 * Empty Courses State
 */
export const EmptyCoursesState = memo<{
  onBrowseCourses?: () => void
  onCreateCourse?: () => void
  userRole?: string
  className?: string
}>(({ onBrowseCourses, onCreateCourse, userRole = 'student', className }) => {
 ...
 */
export const EmptyAssignmentsState = memo<{
  onCreateAssignment?: () => void
  onBrowseAssignments?: () => void
  userRole?: string
  className?: string
}>(({ onCreateAssignment, onBrowseAssignments, userRole = 'student', className }) => {
  const isTeacher = userRole === 'teacher' || userRole === 'admin'
    ...
    <EmptyState
      ...
      description={
        isTeacher
          ...
          : "No assignments are currently available. Check back later or contact your instructor."
      }
      action={
 ...
 */
export const EmptyDiscussionsState = memo<{
  onCreateDiscussion?: () => void
  onBrowseDiscussions?: () => void
  userRole?: string
  className?: string
}>(({ onCreateDiscussion, onBrowseDiscussions, userRole = 'student', className }) => {
    ...
    <EmptyState
      ...
      description={
        canCreate
          ...
          : "No discussion topics are available yet. Check back later for new conversations."
      }
      action={
        canCreate && onCreateDiscussion
          ? {
 ...
 */
export const EmptyCalendarState = memo<{
  onCreateEvent?: () => void
  onViewCalendar?: () => void
  className?: string
}>(({ onCreateEvent, onViewCalendar, className }) => {
  return (
    <EmptyState
      icon={Calendar}
      title="No events scheduled"
      description="Your calendar is empty. Add events to stay organized and never miss important dates."
      action={
        onCreateEvent
          ? {
              label: 'Add Event',
 ...
 * Empty Files State
 */
export const EmptyFilesState = memo<{
  onUploadFile?: () => void
  onCreateFolder?: () => void
  canUpload?: boolean
  className?: string
}>(({ onUploadFile, onCreateFolder, canUpload = true, className }) => {
  return (
    <EmptyState
      icon={Folder}
      ...
      description={
        canUpload
          ...
          : "This folder is empty. No files are currently available."
      }
      action={
 ...
 */
export const EmptySearchResultsState = memo<{
  query?: string
  onClearSearch?: () => void
  onModifySearch?: () => void
  className?: string
}>(({ query, onClearSearch, onModifySearch, className }) => {
  return (
    <EmptyState
      icon={Search}
 ...
 */
export const EmptyNotificationsState = memo<{
  onViewSettings?: () => void
  className?: string
}>(({ onViewSettings, className }) => {
  return (
    <EmptyState
      icon={Notification}
 ...
 * Empty Groups State
 */
export const EmptyGroupsState = memo<{
  onJoinGroup?: () => void
  onCreateGroup?: () => void
  userRole?: string
  className?: string
}>(({ onJoinGroup, onCreateGroup, userRole = 'student', className }) => {
    ...
    <EmptyState
      ...
      description={
        canCreate
          ...
          : "You haven't joined any groups yet. Join groups to collaborate with classmates."
      }
      action={
