...
export default function CustomGradebookColumnsPage() {
  ...
  const [colForm, setColForm] = useState({ title: '', hidden: false, teacher_notes: false })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setColForm({ title: '', hidden: false, teacher_notes: false })
      refetch()
    } catch (err: any) {
  ...
  return (
    <div className="cx-page">
      <div className="cx-page__header" style={{ paddingTop: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ margin: 0, fontWeight: 700, color: 'var(--cx-text-primary)' }}>Custom Gradebook Columns</h2>
        <button className="cx-btn cx-btn--primary" onClick={() => { setEditingColumn({}); setColForm({ title: '', hidden: false, teacher_notes: false }) }}>+ New Column</button>
      </div>
            ...
            <tbody>
              {columns.map(c => (
                <tr key={c.id} className="cx-table__row">
                  <td className="cx-table__cell cx-table__cell--name">{c.title}</td>
                  ...
                  <td className="cx-table__cell cx-table__cell--actions">
                    <button className="cx-btn cx-btn--ghost cx-btn--sm" onClick={() => { setEditingColumn(c); setColForm({ title: c.title, hidden: c.hidden, teacher_notes: c.teacher_notes }) }}>Edit</button>
