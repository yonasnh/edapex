...
export default function AttendancePage() {
  ...
  // Locate the Roll Call Attendance assignment
  useEffect(() => {
    if (assignments) {
      const found = assignments.find(a => a.name.toLowerCase() === 'roll call attendance');
      if (found) {
  ...
  const handleInitializeAssignment = async () => {
  ...
  const handleMarkAllPresent = async () => {
    ...
    try {
      const updates = students.map(s => {
        return canvasFetch(`/api/v1/courses/${courseId}/assignments/${attendanceAssignment.id}/submissions/${s.id}`, {
          method: 'PUT',
      ...
      {!attendanceAssignment ? (
        ...
        <div style={{ padding: '24px', background: 'var(--cx-bg-surface)', border: '1px solid var(--cx-border-subtle)', borderRadius: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 24, alignItems: 'center' }}>
            ...
            <button className="cx-btn cx-btn--ghost cx-btn--sm" onClick={handleMarkAllPresent}>Mark All Present</button>
          </div>
