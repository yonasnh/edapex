 ...
 */
export async function canvasApiRoutes(app) {
    ...
    app.post('/api/canvas/token', {
        ...
        const sessionId = request.query?.session_id || request.cookies.lti_session;
        if (!sessionId) {
    ...
    app.register(async function canvasApiProxy(app) {
        app.addHook('preHandler', async (request, reply) => {
            ...
            ;
            request.ltiSession = session;
        });
        // Get user courses
        app.get('/api/canvas/courses', async (request, reply) => {
            try {
                const session = request.ltiSession;
